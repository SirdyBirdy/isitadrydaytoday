...
function submitCrowdsource(locationKey, isDry, dateStr) {
  ...
  if (!CONFIG.CROWDSOURCE_FORM_URL) {
    showToast("Thanks for the report! Connect Google Form in CONFIG to enable live tracking.");
...
function showCrowdDisclaimer(dryCount) {
  ...
  el.innerHTML =
    ...
    " reported a dry day here today. This is <em>not</em> confirmed by official sources. " +
...
function processLocation(countryCode, stateCode, locationLabel, locationKey) {
  ...
  fetchOverride(locationKey, dateStr).then(function(override) {
    ...
    var staticNote  = rule ? rule.note : "We do not have dry day data for your exact location. Check locally.";

    // If static says NOT dry, still check crowd counts for override
    fetchCrowdCounts(locationKey, dateStr).then(function(counts) {
      ...
      if (crowdSaysDry && staticIsDry === false) {
        // Show dry with crowd disclaimer
...
function wireShareButtons(isDry, locationLabel, dateStr) {
  ...
  var emoji  = isDry ? "No entry" : "Beer mug";
  var msg    = (isDry ? "No entry sign Beer mug" : "Beer mug Cheers") +
               " " + status + " in " + locationLabel + " today (" + dateStr +
               "). Check yours at isitadrydaytoday.com";
  var siteUrl = "https://isitadrydaytoday.com";
  ...
  if (im) im.onclick = function() { saveAsImage(); };
}
...
function wireCrowdsourceButtons(locationKey, isDry, dateStr) {
  ...
  if (confirmBtn) confirmBtn.onclick = function() { submitCrowdsource(locationKey, true,  dateStr); };
  if (reportBtn)  reportBtn.onclick  = function() { submitCrowdsource(locationKey, true,  dateStr); };
}
...
function reverseGeocode(lat, lon) {
  return fetch(
    ...
    { headers: { "User-Agent": "isitadrydaytoday.com/3.0" } }
...
function extractCodes(geo) {
  ...
  if (countryCode === "IN") {
    var stateMap = {
      "maharashtra":"MH","delhi":"DL","national capital territory of delhi":"DL",
      "gujarat":"GJ","bihar":"BR","nagaland":"NA","manipur":"MN","mizoram":"MZ",
      "karnataka":"KA","tamil nadu":"TN","kerala":"KL","uttar pradesh":"UP",
      "west bengal":"WB","rajasthan":"RJ","madhya pradesh":"MP","goa":"GA",
      ...
      "lakshadweep":"LA","andaman and nicobar islands":"AN",
...
function buildStateGrid() {
  ...
  INDIA_STATE_SUMMARY.forEach(function(s) {
    ...
    var icon = status === "dry" ? "no entry" : status === "not-dry" ? "check mark" : "?";
